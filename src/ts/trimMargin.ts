...
const trimMargin = function trimMargin (str: string, delimiter: string = defalutDelimiter): string {
...
const tm = function tm (strings: string[], ...values: string[]): string {
	...
	return trimMargin(
		strings.reduce(
			(prev: string, current: string, index: number) => prev + values[index - 1] + current));
};
